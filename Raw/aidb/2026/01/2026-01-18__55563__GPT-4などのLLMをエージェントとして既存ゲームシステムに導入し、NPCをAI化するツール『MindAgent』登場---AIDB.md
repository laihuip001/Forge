...
*   URL：[https://doi.org/10.48550/arXiv.2309.09971](https://doi.org/10.48550/arXiv.2309.09971)
*   PJページ：[https://mindagent.github.io/](https://mindagent.github.io/)

https://twitter.com/ai\_database/status/1704697405478990201
...
![](https://ai-data-base.com/wp-content/uploads/2023/09/AIDB_55563_1-1024x683.jpg)
...
![](https://ai-data-base.com/wp-content/uploads/2023/09/AIDB_55563_4.jpg)
...
![](https://ai-data-base.com/wp-content/uploads/2023/09/AIDB_55563_5.jpg)
...
1.  エージェントが指定された場所に移動（`goto(agent, location)`）
2.  特定の種類のモブ（敵キャラクター）を倒す（`killMob(agent, mobType)`）
3.  特定の種類のブロックを採掘（`mineBlock(agent, blockType)`）
4.  燃料を炉に入れる（`putFuelFurnace(agent, fuelType)`）
5.  アイテムを炉に入れる（`putItemFurnace(agent, itemType)`）
6.  調理されたアイテムを炉から取り出す（`takeOutFurnace(agent)`）
7.  アイテムをチェストに入れる（`putInChest(agent, itemType)`）
